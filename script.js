...
function openGame(gameUrl) {
    // Open game URL directly in new tab
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch() {
        const searchTerm = gameSearch.value.toLowerCase().trim().replace(/\s+/g, '');
        ...
        gameCards.forEach((card, index) => {
            ...
            const gameTitle = card.querySelector('h3')?.textContent.toLowerCase().replace(/\s+/g, '') || '';
            // Get game description (assuming it's in a p element)
            const gameDescription = card.querySelector('p')?.textContent.toLowerCase().replace(/\s+/g, '') || '';
